...
function Write-Response($id, $state, $errorMessage = $null) {
    $payload = @{
        id = $id
        ok = [string]::IsNullOrEmpty($errorMessage)
    }
...
while ($true) {
    ...
    try {
        ...
        switch ($request.command) {
            "get" {
                Write-Response $request.id (Get-StateObject)
            }
            "set-volume" {
                ...
                [AudioDeck.AudioManager]::SetMasterVolume($level / 100.0)
                Write-Response $request.id (Get-StateObject)
            }
            "set-mute" {
                [AudioDeck.AudioManager]::SetMute([bool]$request.muted)
                Write-Response $request.id (Get-StateObject)
            }
            default {
        ...
        $requestId = if ($request -and $request.id) { $request.id } else { $null }
        Write-Response $requestId $null $_.Exception.Message
    }
}
